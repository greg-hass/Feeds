...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, Image, Animated, Platform } from 'react-native';
...
export default function BookmarksList({ onArticlePress, activeArticleId, isMobile = false }: BookmarksListProps) {
    const colors = useColors();
    const router = useRouter();
    const { bookmarkedArticles, fetchBookmarks, isLoading, markRead } = useArticleStore();
    ...
    const renderArticle = ({ item }: { item: Article }) => {
        ...
        return (
            <TouchableOpacity
                style={[s.articleCard, item.is_read && s.articleRead, isActive && s.articleActive]}
