...
export const useTimeline = (onArticlePress?: (article: Article) => void) => {
    ...
    // Track when refresh completes to display "Last refreshed"
    useEffect(() => {
        if (!isRefreshing && lastRefreshed === null) {
            setLastRefreshed(new Date());
        }
        if (isRefreshing === false && lastRefreshed !== null && (useFeedStore.getState().isLoading === false)) {
            setLastRefreshed(new Date());
        }
    }, [isRefreshing]);
    ...
    return {
        articles, isLoading, hasMore, filter, feeds, isFeedLoading, headerTitle, lastRefreshed, isRefreshing, refreshProgress,
