...
export default function ManageScreen() {
  ...
  const [expandedDiscoveries, setExpandedDiscoveries] = useState<Set<string>>(
    new Set(),
  );
  const [previewArticles, setPreviewArticles] = useState<
    Record<string, FeedPreview[]>
  >({});
  ...
  const filteredFeeds = useMemo(() => {
    ...
    return feeds.filter((feed) => {
      const folderName = feed.folder_id
        ? folderNameById.get(feed.folder_id)
        : null;
      const haystacks = [feed.title, feed.url, feed.type, folderName].filter(
        Boolean,
      ) as string[];
      return haystacks.some((value) => value.toLowerCase().includes(query));
    });
  }, [feedSearch, feeds, folderNameById]);
  const visibleFeedCount = feedSearch.trim()
  ...
  const handleAddFeed = useCallback(
    async (discovery: DiscoveredFeed) => {
      ...
      try {
        ...
        const suggestedFolder = suggestFolderName(
          discovery.type,
          discovery.title,
        );
        ...
        if (folderId) {
          show(
            `Added "${discovery.title}" to ${suggestFolderName(discovery.type, discovery.title)}`,
            "success",
          );
        } else {
  ...
  const toggleExpandDiscovery = useCallback(
  ...
  const handleRetryFeed = async (feedId: number, feedTitle: string) => {
    try {
      ...
      if (result.success) {
        show(
          `Refreshed "${feedTitle}" - ${result.new_articles} new articles`,
          "success",
        );
        fetchFeeds();
      } else {
      ...
      await api.importOpmlWithProgress(
        ...
        (error: Error) => {
          setProgressState((prev) => ({
            ...
            failedFeeds: [
              ...prev.failedFeeds,
              { id: 0, title: "Import", error: error.message },
            ],
          }));
        },
      );
    ...
    await api.refreshFeedsWithProgress(
      ...
      (error: Error) => {
        setProgressState((prev) => ({
          ...
          failedFeeds: [
            ...prev.failedFeeds,
            { id: 0, title: "Refresh", error: error.message },
          ],
        }));
      },
    );
    ...
    await api.refreshFeedsWithProgress(
      ...
      (error: Error) => {
        setProgressState((prev) => ({
          ...
          failedFeeds: [
            ...prev.failedFeeds,
            { id: 0, title: "Retry", error: error.message },
          ],
        }));
      },
    );
  ...
  const getTypeIcon = (type: Feed["type"]) => {
    switch (type) {
      ...
      case "podcast":
        return <Headphones size={18} color={colors.secondary.DEFAULT} />;
      case "reddit":
        return <MessageSquare size={18} color="#f97316" />;
      default:
        return (
          <Rss size={18} color={colors.primary?.DEFAULT ?? colors.primary} />
        );
    }
  };
                      ...
                      <View style={s.groupSection}>
                        ...
                        {grouped.rss.map((discovery, i) => (
                          <DiscoveryCard
                            ...
                            discovery={discovery}
                            previewArticles={
                              previewArticles[discovery.feed_url] || []
                            }
                            ...
                            onOpenSite={() =>
                              handleOpenSite(
                                discovery.site_url || discovery.feed_url,
                              )
                            }
                            expanded={expandedDiscoveries.has(
                              discovery.feed_url,
                            )}
                            onToggleExpand={() =>
                              toggleExpandDiscovery(discovery.feed_url)
                            }
                          />
                        ))}
                      </View>
                    ...
                    {grouped.reddit.length > 0 && (
                      <View style={s.groupSection}>
                        <View style={s.groupHeader}>
                          <MessageSquare
                            size={18}
                            color={colors.feedTypes.reddit}
                          />
                          <Text style={s.groupTitle}>Subreddits</Text>
                          <Text style={s.groupCount}>
                            {grouped.reddit.length}
                          </Text>
                        </View>
                          ...
                          >
                              ...
                              <View style={s.feedIconContainer}>
                                {feed.icon_url ? (
                                  <Image
                                    ...
                                    style={[
                                      s.feedIcon,
                                      feed.paused_at && s.feedIconPaused,
                                    ]}
                                  />
                                ) : (
                            ...
                            <View style={s.feedActions}>
                              {feed.error_count > 0 ? (
                                <TouchableOpacity
                                  onPress={() =>
                                    handleRetryFeed(feed.id, feed.title)
                                  }
                                  style={s.actionButton}
                                >
        ...
        <View style={s.section}>
          ...
          <Button
            ...
            style={{
              ...
              backgroundColor: colors.primary?.dark ?? colors.primary,
            }}
          />
        </View>
      </ScrollView>
        ...
        <View style={s.bulkToolbar}>
          ...
          <View style={s.bulkActions}>
            ...
            >
              ...
              <Text
                style={[s.bulkButtonText, { color: colors.secondary.DEFAULT }]}
              >
                Move
              </Text>
            </TouchableOpacity>
          ...
          <View style={s.modal}>
            ...
            <View style={s.modalActions}>
              <Button
                ...
                style={{ flex: 1 }}
              />
              <Button title="Save" onPress={submitRename} style={{ flex: 1 }} />
            </View>
          </View>
        </View>
      </Modal>
          ...
          <View style={s.moveModalContainer}>
            ...
            >
              ...
              >
                ...
                <Text
                  style={[
                    s.moveModalOptionText,
                    selectedFolderId === null && s.moveModalOptionTextSelected,
                  ]}
                >
                  No Folder
                </Text>
                    ...
                    .map((feed: Feed) => (
                      ...
                      >
                        {feed.icon_url ? (
                          <Image
                            source={{ uri: feed.icon_url }}
                            style={s.viewFolderFeedIcon}
                          />
                        ) : (
                          <View
                            style={[
                              s.viewFolderFeedIconPlaceholder,
                              { backgroundColor: colors.background.tertiary },
                            ]}
                          >
                        ...
                        <View style={s.viewFolderFeedInfo}>
                          ...
                          <Text style={s.viewFolderFeedType}>{feed.type}</Text>
                        </View>
...
const styles = (colors: any) =>
  StyleSheet.create({
    ...
    folderContent: {
      // New style for folder content layout
      flex: 1,
      flexDirection: "row",
      alignItems: "center",
      padding: spacing.md,
    ...
    feedActions: {
      // New style for the action buttons container
      flexDirection: "row",
      alignItems: "center",
